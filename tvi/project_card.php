...
/**
 *	\file       htdocs/projet/project_card.php
 *	\ingroup    projet
 *	\brief      Project card
 */
...
$hookmanager->initHooks(array('projectcard_tvi','globalcard'));
...
if (empty($reshook))
{
	...
	if ($action == 'confirm_createfirstaction' && $user->rights->projet->creer && GETPOST('confirm') == 'yes')
	{

		$tvi = new Tvi($db);
		$event_type=$tvi->fetchallperiode();
		...
		if (is_array($event_type) && count($event_type)>0) {
			...
			foreach($event_type as $key=>$value) {
				if ($value->fk_genre == $object->array_options['options_type']) {

					$todo = GETPOST('eventtype_'. $value->fk_typeevent);
					if (!empty($todo)) {
						$done=true;
						$date_dmc=$db->jdate($object->array_options['options_dmc']);
						if (!empty($value->franchise)) {
							$newdate=dol_time_plus_duree($date_dmc, $value->franchise, 'm');
						} else {
							$newdate=dol_time_plus_duree($date_dmc, $value->periode, 'm');
						}
						$event = new ActionComm($db);
						$event->datep = $newdate;
						$event->datef = $newdate;
						$event->percentage = 0;
						$event->fk_project = $object->id;
						$event->type_id=$value->fk_typeevent;
						$event->label=$extrafields->showOutputField('type', $object->array_options['options_type']);
						$event->label.=' '.$extrafields->showOutputField('marque', $object->array_options['options_marque']);
						$event->label.=' '.$object->array_options['options_immat'];
		...
		if ($done && empty($error)) {
			setEventMessage('Actions créées');
			$object->array_options['options_firsteventdone']=1;
			$object->update($user);
		}
	}
}
...
else
{
    ...
    if ($action == 'createfirstaction')
    {
    	$formquestion=array();
    	$formquestion['text']= $langs->trans("Action à créer");
    	...
    	if (is_array($event_type) && count($event_type)>0) {
			foreach($event_type as $key=>$value) {
				if ($value->fk_genre == $object->array_options['options_type']) {
					$label='';
					$label.=$langs->trans($value->libelle);
					$date_dmc=$db->jdate($object->array_options['options_dmc']);
					if (!empty($value->franchise)) {
						$label.= ' - Prochaine échance : '. dol_print_date(dol_time_plus_duree($date_dmc, $value->franchise, 'm'));
					} else {
						$label.= ' - Prochaine échance : '. dol_print_date(dol_time_plus_duree($date_dmc, $value->periode, 'm'));
					}
					$formquestion[]= array('type' => 'checkbox', 'name' => 'eventtype_'.$value->fk_typeevent, 'label' => $label, 'value' => $value->fk_typeevent);
				}
			}
    	}
    	print $form->formconfirm($_SERVER["PHP_SELF"]."?id=".$object->id, $langs->trans("Action à créer"), $langs->trans("Action à créer"), "confirm_createfirstaction", $formquestion, '', 1, 280);
    }
    ...
    else
    {
        ...
        // Ref
        print '<tr class="liste_titre"><td Width="15%">'.$extralabels['immat'] .'</td><td align ="center" Width="35%">' . $extrafields->showOutputField('immat', $object->array_options['options_immat']) . '</td><td Width="15%">Numéro de parc :</td><td align ="center" Width="35%">';
        ...
        // Label
        print '<tr><td Width="15%">Descrpition</td><td colspan="3">'.$object->title.'</td></tr>';

        // Date start
        print '<tr><td Width="15%">Date de création</td><td Width="35%">';
        print dol_print_date($object->date_start,'daytext');
        print '</td><td Width="15%">Date de désactivation</td><td Width="35%">';
        print dol_print_date($object->date_end,'daytext');
        print '</td></tr>';

        // Statut
        print '<tr><td>'.$langs->trans("Status").'</td><td>'.$object->getLibStatut(4).'</td>';
        print '<td>Statut du Véhicule</td><td>';
        $code = dol_getIdFromCode($db, $object->opp_status, 'c_lead_status', 'rowid', 'code');
        if ($code) print $langs->trans("OppStatus".$code);
        print '</td></tr>';

        print '<tr class="liste_titre"><th colspan ="4" align="center"> Description technique du véhicule</th></tr>';

        print '<tr><td Width="15%">'.$extralabels['type'] . '</td><td Width="35%">';
        print $extrafields->showOutputField('type', $object->array_options['options_type']);
        print '</td><td Width="15%">'.$extralabels['marque'] . '</td><td Width="35%">';
        print $extrafields->showOutputField('marque', $object->array_options['options_marque']);
        print '</td></tr>';

        print '<tr><td Width="15%">'.$extralabels['modele'] . '</td><td Width="35%">';
        print $extrafields->showOutputField('modele', $object->array_options['options_modele']);
        print '</td><td Width="15%">'.$extralabels['carrosserie'] . '</td><td Width="35%">';
        print $extrafields->showOutputField('carrosserie', $object->array_options['options_carrosserie']);
        print '</td></tr>';

        print '<tr><td Width="15%">'.$extralabels['puissance'] . '</td><td Width="35%">';
        print $extrafields->showOutputField('puissance', $object->array_options['options_puissance']);
        print ' Ch </td><td Width="15%">'.$extralabels['ptc'] . '</td><td Width="35%">';
        print $extrafields->showOutputField('ptc', $object->array_options['options_ptc']);
        print ' Tonnes</td></tr>';

        print '<tr><td Width="15%">'.$extralabels['silouhette'] . '</td><td Width="35%">';
        print $extrafields->showOutputField('silouhette', $object->array_options['options_silouhette']);
        print '</td><td Width="15%">'.$extralabels['norme'] . '</td><td Width="35%">';
        print $extrafields->showOutputField('norme', $object->array_options['options_norme']);
        print '</td></tr>';
        ...
        print '<tr><td Width="15%">'.$extralabels['chassis'] . '</td><td Width="35%">';
        print $extrafields->showOutputField('chassis', $object->array_options['options_chassis']);
        print '</td><td Width="15%">'.$extralabels['dmc'] . '</td><td Width="35%">';
        print $extrafields->showOutputField('dmc', $object->array_options['options_dmc']);
        print '</td></tr>';

        print '<tr class="liste_titre"><th colspan ="4" align="center">Information diverses</th></tr>';

        print '<tr><td Width="15%">'.$extralabels['sol_trs'] . '</td><td Width="35%">';
        print $extrafields->showOutputField('sol_trs', $object->array_options['options_sol_trs']);
        print '</td><td Width="15%">'.$extralabels['site'] . '</td><td Width="35%">';
        print $extrafields->showOutputField('site', $object->array_options['options_site']);
        print '</td></tr>';

        print '<tr><td Width="15%">'.$extralabels['dt_achat'] . '</td><td Width="35%">';
        print $extrafields->showOutputField('dt_achat', $object->array_options['options_dt_achat']);
        print '</td><td Width="15%">'.$extralabels['categorie'] . '</td><td Width="35%">';
        print $extrafields->showOutputField('categorie', $object->array_options['options_categorie']);
        print '</td></tr>';

        print '<tr><td Width="15%">'.$extralabels['dt_vente'] . '</td><td Width="35%">';
        print $extrafields->showOutputField('dt_vente', $object->array_options['options_dt_vente']);
        print '</td><td Width="15%">'.$extralabels['fac_vente'] . '</td><td Width="35%">';
        print $extrafields->showOutputField('fac_vente', $object->array_options['fac_vente']);
        print '</td></tr>';
    ...
    if (empty($reshook))
    {
	    if ($action != "edit" )
	    {
	        ...
	        if ($object->statut != 2 && $user->rights->projet->creer)
	        {
	            if ($userWrite > 0)
	            {
	                print '<div class="inline-block divButAction"><a class="butAction" href="project_card.php?id='.$object->id.'&amp;action=edit">'.$langs->trans("Modify").'</a></div>';
	            }
	            else
	            {
	        ...
	        if ($object->statut == 0 && $user->rights->projet->creer)
	        {
	            if ($userWrite > 0)
	            {
	                print '<div class="inline-block divButAction"><a class="butAction" href="project_card.php?id='.$object->id.'&action=validate">'.$langs->trans("Validate").'</a></div>';
	            }
	            else
	            {
	        ...
	        if ($object->statut == 1 && $user->rights->projet->creer)
	        {
	            if ($userWrite > 0)
	            {
	                print '<div class="inline-block divButAction"><a class="butAction" href="project_card.php?id='.$object->id.'&amp;action=close">'.$langs->trans("Close").'</a></div>';
	            }
	            else
	            {
	        ...
	        if ($object->statut == 2 && $user->rights->projet->creer)
	        {
	            if ($userWrite > 0)
	            {
	                print '<div class="inline-block divButAction"><a class="butAction" href="project_card.php?id='.$object->id.'&amp;action=reopen">'.$langs->trans("ReOpen").'</a></div>';
	            }
	            else
	            {
	        ...
	        if (! empty($conf->global->PROJECT_SHOW_CREATE_OBJECT_BUTTON))
	        {
	            ...
	            {
	                $langs->load("propal");
	                print '<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.'/comm/propal/project_card.php?action=create&projectid='.$object->id.'&socid='.$object->socid.'">'.$langs->trans("AddProp").'</a></div>';
	            }
	            if (! empty($conf->commande->enabled) && $user->rights->commande->creer)
	            {
	                $langs->load("orders");
	                print '<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.'/commande/project_card.php?action=create&projectid='.$object->id.'&socid='.$object->socid.'">'.$langs->trans("CreateOrder").'</a></div>';
	            }
	            ...
	            if (! empty($conf->supplier_proposal->enabled) && $user->rights->supplier_proposal->creer)
	            {
	                ...
	                print '<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.'/supplier_proposal/project_card.php?action=create&projectid='.$object->id.'&socid='.$object->socid.'">'.$langs->trans("AddSupplierProposal").'</a></div>';
	            }
	            if (! empty($conf->supplier_order->enabled) && $user->rights->fournisseur->commande->creer)
	            {
	                $langs->load("suppliers");
	                print '<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.'/fourn/commande/project_card.php?action=create&projectid='.$object->id.'&socid='.$object->socid.'">'.$langs->trans("AddSupplierOrder").'</a></div>';
	            }
	            if (! empty($conf->supplier_invoice->enabled) && $user->rights->fournisseur->facture->creer)
	            {
	                $langs->load("suppliers");
	                print '<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.'/fourn/facture/project_card.php?action=create&projectid='.$object->id.'&socid='.$object->socid.'">'.$langs->trans("AddSupplierInvoice").'</a></div>';
	            }
	            if (! empty($conf->ficheinter->enabled) && $user->rights->ficheinter->creer)
	            {
	                $langs->load("interventions");
	                print '<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.'/fichinter/project_card.php?action=create&projectid='.$object->id.'&socid='.$object->socid.'">'.$langs->trans("AddIntervention").'</a></div>';
	            }
	            if (! empty($conf->contrat->enabled) && $user->rights->contrat->creer)
	            {
	                $langs->load("contracts");
	                print '<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.'/contrat/project_card.php?action=create&projectid='.$object->id.'&socid='.$object->socid.'">'.$langs->trans("AddContract").'</a></div>';
	            }
	            if (! empty($conf->expensereport->enabled) && $user->rights->expensereport->creer)
	            {
	                $langs->load("trips");
	                print '<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.'/expensereport/project_card.php?action=create&projectid='.$object->id.'&socid='.$object->socid.'">'.$langs->trans("AddTrip").'</a></div>';
	            }
	            if (! empty($conf->don->enabled) && $user->rights->don->creer)
	            {
	                $langs->load("donations");
	                print '<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.'/don/project_card.php?action=create&projectid='.$object->id.'&socid='.$object->socid.'">'.$langs->trans("AddDonation").'</a></div>';
	            }
	        }
	        ...
	        if ($user->rights->projet->creer)
	        {
	            if ($userWrite > 0)
	            {
	                print '<div class="inline-block divButAction"><a class="butAction" href="project_card.php?id='.$object->id.'&action=clone">'.$langs->trans('ToClone').'</a></div>';
	            }
	            else
	            {
	        ...
	        //Création evenement controle camion
	        if ($user->rights->tvi->creeraction && empty($object->array_options['options_firsteventdone']) && !empty($object->array_options['options_dmc']))
	        {
	        	print '<div class="inline-block divButAction"><a class="butAction" href="'.dol_buildpath('/tvi/tvi/project_card.php',2).'?id='.$object->id.'&amp;action=createfirstaction">'.$langs->trans("Calendrier initale des controles").'</a></div>';
	        }
	        ...
	        if ($user->rights->projet->supprimer || ($object->statut == 0 && $user->rights->projet->creer))
	        {
	            ...
	            {
	                print '<div class="inline-block divButAction"><a class="butActionDelete" href="project_card.php?id='.$object->id.'&amp;action=delete">'.$langs->trans("Delete").'</a></div>';
	            }
	            else
	            {
